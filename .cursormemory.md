...
- Playwright coverage is enabled with `PW_COVERAGE=true` and writes reports to `functional_tests/coverage-report/`.
...
- Logs are emitted as JSON to stdout and shipped to Grafana via Loki + Promtail from Docker container logs.
...
## Configuration Notes
- JWT and admin bootstrap settings are required environment variables and are documented in `env.local` and `env.example`.
- Tests seed required environment variables in `backend/tests/test_env.py` before loading settings.
- Local Docker Compose uses `env.local` via `ENV_FILE`, and `env.example` is the template for new environments.
